...
class interface(Tk):
    ...
    def subir_variavel(self, n):
        if n > 0:
            self.minimo = self.dados_variaveis[n][0].get()
            self.nome = self.dados_variaveis[n][1].get()
            self.maximo = self.dados_variaveis[n][2].get()
            self.tipo = self.dados_variaveis[n][3].get()

            self.dados_variaveis[n][0].set(self.dados_variaveis[n-1][0].get())
            self.dados_variaveis[n][1].set(self.dados_variaveis[n-1][1].get())
            self.dados_variaveis[n][2].set(self.dados_variaveis[n-1][2].get())
            self.dados_variaveis[n][3].set(self.dados_variaveis[n-1][3].get())

            self.dados_variaveis[n-1][0].set(self.minimo)
            self.dados_variaveis[n-1][1].set(self.nome)
            self.dados_variaveis[n-1][2].set(self.maximo)
            self.dados_variaveis[n-1][3].set(self.tipo)


    def descer_variavel(self, n):
        if n < int(self.num_variaveis.get()):
            self.minimo = self.dados_variaveis[n][0].get()
            self.nome = self.dados_variaveis[n][1].get()
            self.maximo = self.dados_variaveis[n][2].get()
            self.tipo = self.dados_variaveis[n][3].get()
    ...
    def lista_variaveis(self):
        for i in range(0,int(self.num_variaveis.get())):
            self.dados_variaveis.append([])
            for j in range(0,4):
                self.dados_variaveis[i].append(StringVar())
        for i in range(0,int(self.num_variaveis.get())):
            self.lista_entry_variaveis.append([])
            self.lista_entry_variaveis[i].append(Frame(self.tabela_var, width = 10, height = 10, bg = "blue"))
            self.lista_entry_variaveis[i][0].grid(row=i+1, column = 0)
            self.buttom_subir = Button(self.lista_entry_variaveis[i][0], text = "^", command=partial(self.subir_variavel,i))
            self.buttom_subir.grid(row=0, column = 0)
            self.buttom_descer = Button(self.lista_entry_variaveis[i][0],text = "v", command=partial(self.descer_variavel,i))
            self.buttom_descer.grid(row=1, column = 0)
            self.lista_entry_variaveis[i].append(Entry(self.tabela_var, textvariable= self.dados_variaveis[i][0]))
            self.lista_entry_variaveis[i][1].grid(row=i+1, column = 1)
            self.lista_entry_variaveis[i].append(Entry(self.tabela_var, textvariable= self.dados_variaveis[i][1]))
            self.lista_entry_variaveis[i][2].grid(row=i+1, column = 2)
            ...
            self.lista_entry_variaveis[i][4].grid(row=i+1, column = 4)

    """
    def subir_variavel_saida(self, n):
        if n > 0:

            self.nome_saida = self.dados_variaveis_saida[n].get()

            self.dados_variaveis_saida[n].set(self.dados_variaveis_saida[n-1].get())

            self.dados_variaveis_saida[n-1].set(self.nome_saida)


    def descer_variavel_saida(self, n):
        if n < int(self.num_variaveis_saida.get()):
    ...
    def lista_variaveis_saida(self):
        for i in range(0,int(self.num_variaveis_saida.get())):
            self.dados_variaveis_saida.append(StringVar())
        for i in range(0,int(self.num_variaveis_saida.get())):
            self.Area_botao_saida = Frame(self.tabela_var_saida, width = 10, height = 10)
            self.Area_botao_saida.grid(row=i+1, column = 0)
            self.buttom_subir_saida = Button(self.Area_botao_saida, text = "^", command=partial(self.subir_variavel_saida,i))
            self.buttom_subir_saida.grid(row=0, column = 0)
            self.buttom_descer_saida = Button(self.Area_botao_saida,text = "v", command=partial(self.descer_variavel_saida,i))
            self.buttom_descer_saida.grid(row=1, column = 0)
            self.lista_entry_variaveis_saida.append(Entry(self.tabela_var_saida, textvariable= self.dados_variaveis_saida[i]))
            self.lista_entry_variaveis_saida[i].grid(row=i+1, column = 1)
